...
export default function validateUsername(value) {
    ...
    return (isValidUsername ? `Hello ${value}!` : 'Please enter a username using only letters, numbers, and dashes');
}
...
export default function validatePassword(value) {
    ...
    return (isValidPassword ? 'Password Success' : 'Please enter a password that includes at least 6 characters with one uppercase letter, one lowercase letter, one number, and one special character.')
}